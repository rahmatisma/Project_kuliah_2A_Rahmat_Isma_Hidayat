...
if (!empty($_POST['input_ticket_validate'])) {
    $select = mysqli_query($conn, "SELECT * FROM tb_ticket WHERE nama_film = '$nama_film' AND theater = '$theater' AND jam_tayang = '$jam_tayang' AND id_ticket != '$id_ticket' ORDER BY jam_tayang");
    if (mysqli_num_rows($select) > 0) {
        $message = '<script>alert("Ticket dengan nama film dan theater yang sama telah ada");
                        window.location="../Ticket"</script>';
    } else {
        $query = mysqli_query($conn, "UPDATE tb_ticket SET nama_film = '$nama_film', theater = '$theater', jam_tayang = '$jam_tayang' WHERE id_ticket = '$id_ticket'");
        if ($query) {
